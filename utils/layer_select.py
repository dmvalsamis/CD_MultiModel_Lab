...
def feature_selector_simple(depth, pretext_model, cd_model, trainable=True):
    depth = depth
    ...
    for i in range(len(cd_model.layers)):
        ...
        if cd_model.layers[i].name == 'norm_1' and depth >= 1:
        ...
        if cd_model.layers[i].name == 'norm_2' and depth >= 1:
            cd_model.layers[i].set_weights(sim_model.layers[2].layers[1].get_weights())
            cd_model.layers[i].trainable = trainable
            print(i, cd_model.layers[i].name, sim_model.layers[2].layers[1].name)
        ...
        # if cd_model.layers[i].name == 'norm1_1' and depth >= 1:
        #     cd_model.layers[i].set_weights(sim_model.layers[2].layers[3].get_weights())
        #     cd_model.layers[i].trainable = trainable
        #     print(i, cd_model.layers[i].name, sim_model.layers[2].layers[3].name)
        # if cd_model.layers[i].name == 'norm1_2' and depth >= 1:
        #     cd_model.layers[i].set_weights(sim_model.layers[2].layers[3].get_weights())
        #     cd_model.layers[i].trainable = trainable
        #     print(i, cd_model.layers[i].name, sim_model.layers[2].layers[3].name)
        ...
        # if cd_model.layers[i].name == 'norm2_1' and depth >= 2:
        #     cd_model.layers[i].set_weights(sim_model.layers[2].layers[5].get_weights())
        #     cd_model.layers[i].trainable = trainable
        #     print(i, cd_model.layers[i].name, sim_model.layers[2].layers[5].name)
        # if cd_model.layers[i].name == 'norm2_2' and depth >= 1:
        #     cd_model.layers[i].set_weights(sim_model.layers[2].layers[5].get_weights())
        #     cd_model.layers[i].trainable = trainable
        #     print(i, cd_model.layers[i].name, sim_model.layers[2].layers[5].name)
    ...
    for i in range(len(cd_model.layers)):
        if cd_model.layers[i].name == 'norm1_1' and depth >= 1:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
        if cd_model.layers[i].name == 'dropout1_1' and depth >= 1:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
        if cd_model.layers[i].name == 'relu1_1' and depth >= 1:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
        ...
        if cd_model.layers[i].name == 'norm2_1' and depth >= 2:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
        if cd_model.layers[i].name == 'dropout2_1' and depth >= 2:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
        if cd_model.layers[i].name == 'relu2_1' and depth >= 2:
            cd_model.layers[i].trainable = True
            print(i, cd_model.layers[i].name, "weights frozen")
...
def transfer_learning_model(pretrained_model, new_model, trainable=False):
    # Loop through layers in the new model
    for i in range(len(new_model.layers)):
        layer_name = new_model.layers[i].name
        
        # Check if the layer exists in the pretrained model
        if layer_name in pretrained_model.layers:
            # Transfer weights from the corresponding layer in the pretrained model to the new model
            pretrained_layer = pretrained_model.get_layer(layer_name)
            new_model.layers[i].set_weights(pretrained_layer.get_weights())
            
            # Optionally set the layer as trainable or non-trainable
            new_model.layers[i].trainable = trainable
            
            print(f"Transferred weights from {layer_name} of the pretrained model to {layer_name} of the new model.")
        else:
            print(f"Layer {layer_name} not found in the pretrained model. Skipping.")
