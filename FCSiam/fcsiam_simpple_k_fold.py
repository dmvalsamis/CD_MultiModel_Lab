...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_data)):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs1, inputs2, labels) in enumerate(train_loader):
            ...
            print("Inputs on GPU:", inputs1.is_cuda, inputs2.is_cuda)
            optimizer.zero_grad()
    ...
    with torch.no_grad():
        for inputs1, inputs2, labels in val_loader:
            inputs1, inputs2, labels = inputs1.to(device), inputs2.to(device), labels.to(device)
            print("Validation Inputs on GPU:", inputs1.is_cuda, inputs2.is_cuda)
            outputs = model(torch.stack((inputs1, inputs2), dim=1))
...
log_params_sim1("k-fold", " ", ' ', " ", " ", "Softmax", " ", 'Adam', num_epochs, 'weighted_categorical_crossentropy', " ", 
                avg_metrics['recall'], avg_metrics['specificity'], avg_metrics['precision'], avg_metrics['f1'], avg_metrics['accuracy'], 
                "CBMI Set", 96, " ", "none", cd_model_name)
