...
def change_detection(dataset_name, input_rasters, model, device):
    ...
    # Save the prediction
    predictions_path = f'/data/valsamis_data/data/CBMI/CBMI_0.3/Predictions/Depth_2/gasnet/{dataset_name}'
    ...
    ax[2].set_title('GasNet Prediction', fontsize=font)
    ax[2].axis('off')
...
with open(dataset_names_file, 'r') as file:
    dataset_names_line = file.readline().strip()  
    dataset_names = dataset_names_line.split(',')  
...
model_path = '/home/dvalsamis/Documents/projects/Change_detection_SSL_Siamese/saved_models_Torch/Gas_Net_CBMI_e648.h5'
