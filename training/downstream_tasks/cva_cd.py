...
#     img1 = expand_dims(X1[index], axis=0)
#     img2 = expand_dims(X2[index], axis=0)
    
#     feature_maps_left = branch_model.predict(img1)
#     feature_maps_right = branch_model.predict(img2)

#     left = np.ndarray(shape=(32,ImageSize,ImageSize))
#     right = np.ndarray(shape=(32,ImageSize,ImageSize))
...
#     if FILTER == True:
#        distmap = gaussian(distmap, sigma=1, mode='constant', cval=0.0)
...
#New method with ASPP WORKING one Branch --------------------------------------------------------------------------------------------------------------------------------------------
# FILTER = False
...
#     # Reduce the dimensionality from (96, 96, 32) to (96, 96) by averaging across channels
#     feature_maps_left = np.mean(feature_maps_left, axis=-1)
#     feature_maps_right = np.mean(feature_maps_right, axis=-1)
    
#     # Now, you have a single-channel (96, 96) distance map
#     distmap = np.abs(feature_maps_left - feature_maps_right)  # absolute difference between averaged maps
    
#     if FILTER:
#         distmap = gaussian(distmap, sigma=1, mode='constant', cval=0.0)
    
#     # Apply Otsu and Triangle thresholding
#     binary_otsu = distmap > threshold_otsu(distmap)
#     binary_otsu = remove_small_objects(binary_otsu, min_size=100)
