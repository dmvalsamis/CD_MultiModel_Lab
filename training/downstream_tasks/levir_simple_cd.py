...
def visualize_images_and_mask(img1, img2, mask):
    """
    Display two images and their corresponding change mask.
    
    Parameters:
        img1 (numpy.ndarray): First image array.
        img2 (numpy.ndarray): Second image array.
        mask (numpy.ndarray): Change mask array.
    """
...
onera_train_target =  '/data/valsamis_data/data/LEVIR-CD/Levir_NPY/aug_train_data/'  
onera_test_target = '/data/valsamis_data/data/LEVIR-CD/Levir_NPY/aug_test_data/'
...
for index in train.index:
    ...
    # if pos % 10 == 0:  # Adjust this condition to visualize less frequently if needed
    #     visualize_images_and_mask(X1, X2, y_train[pos])
...
before_training_weights = get_layer_weights(cd_model, ["norm_1", "conv1_1", "norm1_1", "relu1_1", "dropout1_1", "conv2_1", "norm2_1", "relu2_1", "dropout2_1"])
...
after_training_weights = get_layer_weights(cd_model, ["norm_1", "conv1_1", "norm1_1", "relu1_1", "dropout1_1", "conv2_1", "norm2_1", "relu2_1", "dropout2_1"])

# Compare weights
compare_weights(before_training_weights, after_training_weights)
...
metrics
'''
#y_true = np.ndarray(shape=(y_pred.shape))
#for i in range(y_true.shape[0]):
#    y_true[i] = np.load(onera_test_target+onera_test_df['change_mask'][i]
...
cd_model_name = "CD_Simple_"+"Levir_"+model_id+".h5"

model_path = '/home/dvalsamis/Documents/projects/Change_detection_SSL_Siamese/saved_models/'
#cd_model_name = "03_conv_classifier_two_drop0.1_abs_diff_50epochs_weighted0.1_4.5.h5"
cd_model.save_weights(model_path+cd_model_name)
print("Saved model to disk")

weight_par = '[' + str(wx) + ',' + str(wy) + ']'
log_params_sim1("Task 1", "Linear", 'ASPP', weight_par, depth, "Softmax", LEARNING_RATE, 'Adam', EPOCHS, 'weighted_categorical_crossentropy', BATCH_SIZE, rec, spec, prec, f_m, acc, "Levir-CD Set", 96, NORM, pretext_model,cd_model_name)
